...
fn main() {
    ...
    for x in 1..grid[0].len() - 2 {
        ...
        starting_beams.push(Beam {
            pos: (x as isize, grid.len() as isize - 1),
            dir: Dir::Up,
            complete: false,
        });
    }
    for y in 1..grid.len() - 2 {
        ...
        starting_beams.push(Beam {
            pos: (grid[0].len() as isize - 1, y as isize),
            dir: Dir::Left,
            complete: false,
        });
    }
    ...
    let corners = vec![
        ...
        Beam {
            pos: (grid[0].len() as isize - 1, grid.len() as isize - 1),
            dir: Dir::Left,
            complete: false,
        },
        Beam {
    ...
    let beams = [starting_beams, corners].concat();

    let out = beams
        .iter()
        .map(|beam| energised_for_beams(&grid, beam.clone()))
        .max()
        .unwrap();
...
fn energised_for_beams(grid: &Vec<Vec<char>>, beam: Beam) -> usize {
